...
# Install packages
apt-get -y install openvpn easy-rsa mailutils postfix
...
firewall-cmd --permanent --direct --passthrough ipv4 -t nat -A POSTROUTING -s 10.8.0.0/24 -o "$interface" -j MASQUERADE
firewall-cmd --permanent --direct --passthrough ipv4 -t nat -A POSTROUTING -s 10.8.1.0/24 -o "$interface" -j MASQUERADE
firewall-cmd --reload
...
if [[ $adblock == "yes" ]]; then
  ...
  echo "# Blocked Domains" >>/etc/hosts
